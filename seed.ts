...
import { Franchise } from "./dbschema/edgeql-js/modules/default";
...
async function run() {
  ...
  try {
    ...
    for (const actor of actorNames) {
      await e.insert(e.Person, { name: actor, gender: "Male" }).run(client);
    }

    console.log(`Creating franchise Marvel Cinematic Universe...`);
    const newFranchise = await e
      .insert(e.Franchise, {
    ...
    for (const movie of fullMovies) {
      ...
      const newMovie = await e
        .insert(e.Movie, {
          ...
          franchise: e.select(e.Franchise, (franchise) => ({
            filter_single: {
              id: newFranchise.id,
            },
          })),
